...
function TopoFactory(JTopo){
    ...
    function createLink (beginNode, endNode) {
        ...
        link.click(function (event) {
        		document.getElementById('link-attribute').classList.add('layui-show');
        		document.getElementById('node-attribute').classList.remove('layui-show');
        		document.getElementById("link-color").value=this.strokeColor;
            document.getElementById("link-width").value=this.lineWidth;
    ...
    function createNode (img_path,uuid,nodeType, hmi) {
        ...
        if(hmi){
            ...
            node.click(function (event) {
            		document.getElementById('node-attribute').classList.add('layui-show')
        				document.getElementById('link-attribute').classList.remove('layui-show')
        				
                document.getElementById("text").value=this.text;
                document.getElementById("type").value=this.nodeType;
...
//GTopo图元组合类--Group   JTopo.GenerateGroup=GenerateGroup       liuxf 2019.09.04
function (JTopo) {
        function b(c,json) {
...
function HMI2() {
    ...
    //初始化场景 private
    this.initScene=function(canvas, textfield,
                             jsonToplogy) {
...
//				this.stage.wheelZoom = 0
        ...
        //单击关闭菜单 zwh
        self.scene.click(function (event) {
        			self.nodeMenu.hide();
        			self.mainMenu.hide();
        			self.lineMenu.hide();
        ...
        function mouseupLink(e){
            ...
            if (e.target != null && e.target instanceof JTopo.Node) {
                if (beginNode == null) {
                    ...
                    //线已经画好了，把所有属性变回的
                    beginNode = null;
                    self.scene.remove(link);//移除场景的某个元素
                    link.clear();
                }
                else {
                    beginNode = null;
        ...
        this.scene.mouseover(function (event) {
            ...
            if (event.target != null && event.target instanceof JTopo.Node) {
                ...
                // 进入某个连线
            } else if (event.target != null && event.target instanceof JTopo.Link && event.target.linkTooltip && hmi.stageMode !== 'edit') {
    ...
    this.initMenus = function () {
        ...
        self.nodeMenu.on('click', function (event) {
            ...
            if (text === '删除节点(Delete)') {
                hmi.utils.deleteSelectedNodesLinks()
            ...
            switch (text) {
                ...
                case '逆时针旋转(Shift+I)':
                    ...
                    break
                case '节点设置':
                		self.utils.settingSelectedNodes()
                    self.utils.saveNodeNewState()
                    break
                case '节点信息':
                		self.utils.NodeTip()
    ...
    //从json加载文件 private
    this.createStageFromJson= function (jsonStr, canvas) {
    ...
    //保存Group图元信息 public
    this.saveGroupEle=function (groupName) {
     ...
     * @param drawArea 舞台区域
     */
    this.drag = function (modeDiv, drawArea, text) {
        if (!text) text = ''
        let self = this
        // 拖拽开始,携带必要的参数
        ...
        drawArea.ondrop = function (event) {
            ...
            if (data) {
                ...
                if (datas && datas.length === 3) {
                    ...
                    //zwh 新增alarm
                    node.alarmFlag = 'ture';    // 在创建节点的时候添加自定义属性   
        						node.serializedProperties.push("alarmFlag"); // 把自定义属性加入进节点的属性组 serializedProperties 
    ...
    /**
     * 加载指定id的拓扑图JSON数据结构  public
     * @param topologyGuid 拓扑 表记录ID
     * @param backImg 拓扑图的背景图片
     */
    this.loadTopology = function (url, canvas,textField) {
        let self=this;
        $.ajax({
            type: 'get',
            url: url,
...
//    }
    ...
    // 删除选择的节点
    deleteSelectedNodesLinks: function () {
        ...
        if (nodes && nodes.length > 0) {
            for (let i = 0; i < nodes.length; i++) {
            	if (nodes[i].elementType == 'node'){
              	...
              	self.deleteLine(nodes[i])
              }
            }
        }
    },
    // 选择设置节点 zwh
    settingSelectedNodes: function () {
					if (hmi.stageMode !== 'edit') {
            return false
	        }
	        let self = this
    ...
    settingNode:function (node) {
    	...
    	var height = document.getElementById('height').value;
			...
			node.text = text;
			node.nodeType = type;
			node.width = parseInt(width);
			node.height = parseInt(height);
		},
    ...
    NodeTip: function  () {
			  ...
			  nodes.forEach(function (n) {
			  	if (n.elementType === 'node') {
							layer.open({
						  	title: '节点信息'
						  	,content: '名字 ： '+n.text+'<br>'+'类型: '+n.nodeType+'<br>'+'坐标x：'+n.x+'坐标y：'+n.y+'<br>'+'宽 ： '+n.width+'长 ： '+n.height
							});  	
			  	}
				})
		},
		
		NodeTip2: function(node) {
			 
	            		document.getElementById("text").value=node.text;
						      document.getElementById("type").value=node.nodeType;
						      document.getElementById("x").value=node.x;
									document.getElementById("y").value=node.y;
									document.getElementById('width').value=node.width;
									document.getElementById('height').value=node.height;
	              
		},
		settingSelectedLinks: function () {
					if (hmi.stageMode !== 'edit') {
            return false
	        }
	        let self = this
	        let links = hmi.scene.selectedElements
	        if (links && links.length > 0) {
	            for (let i = 0; i < links.length; i++) {
	                self.settingLink(links[i]);
	            }
	        }
	   },
	  settingLink:function (link) {
	  	var lineWidth = document.getElementById('link-width').value;
	  	link.strokeColor = document.getElementById('link-color').value;
    	link.lineWidth = parseInt(lineWidth);
    	link.text = '';
		},  
		//报警 zwh
    Alarm: function () {
        let nodes = hmi.scene.selectedElements
		      	nodes.forEach(function (n) {
                	 if (n.elementType === 'node' && n.alarmFlag == 'ture') {
                	 	console.log('1');
                	 	n.alarm = 'a';
                	 }
                });	
